...
## Trasporto: interruzioni e UIOLI
...
La schermata **Misure dei PDR** porta nel portale le misure pubblicate dal distributore su SIICloud, il cloud Nextcloud gestito da Acquirente Unico. Si incollano l'indirizzo WebDAV, utente e password; il modulo elenca file e sottocartelle della cartella indicata (protocollo WebDAV standard, solo libreria Python), riconosce i flussi reali dal nome dell'archivio — **TGL letture giornaliere, TMV e SWG1 letture mensili, IGMG cambio contatore** — e apre il file scelto (ZIP con un solo XML) riassumendone il contenuto. Con l'azione «serie» interpreta i tracciati, calcola i consumi giornalieri dalle letture cumulative (il cambio contatore IGMG diventa la nuova base) e passa la serie direttamente alla **previsione della domanda**. Su scelta dell'operatore l'accesso può essere **salvato nel solo database locale**: il portale allora sincronizza ogni giorno i file nuovi in un archivio accanto al database, e la serie si può ricalcolare dall'archivio anche senza rete. Di base le credenziali restano usa e getta e la password salvata non torna mai al frontend. Dettagli in [docs/misure.md](docs/misure.md).
